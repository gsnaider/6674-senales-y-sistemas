...
Y_eje = 0:2*pi/length(Y):2*pi*(length(Y) - 1)/length(Y); % defino el eje x para el grafico
...
%figure, spectrogram(y,w1,200,[],fs,'yaxis')  % senial, ventana(vector de muestras), cada cuantas muestras se solapa, cuantas muestras hace la transformada, frecuencia para poder plotear 
figure, specgram(y,400,fs,w1,200);
